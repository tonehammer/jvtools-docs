...
*This is a test rewrite of the [original page](/advanced-velocity/using/).*

This page covers the setup surface: the six velocity types, the mixer, the output and the visualization. The event workflow gets [its own page](/advanced-velocity/timed-events/).
...
![Basic velocity — every guide trail pointing the same way](/advanced-velocity/static/basic-velocity.webp)
...
* **To Rest Pose** — every point aims at its own captured rest position. This is the reassembly mode: pair it with **Track Motion** and an earlier event that throws the pieces, and each piece gets pulled back to its own spot — see [rest attributes](/advanced-velocity/timed-events/#rest-position-attributes).
...
![Directional velocity — the trails converging on a single target point](/advanced-velocity/static/directional-velocity.webp)
...
![Exploding velocity — an outward burst radiating from the body's centre](/advanced-velocity/static/exploding-velocity.webp)
...
![Velocity from Motion — trails streaming behind a moving object](/advanced-velocity/static/motion-velocity.webp)
...
![Curl Noise velocity — turbulent swirls across a fractured object](/advanced-velocity/static/curl-noise-velocity.webp)
...
![Angular velocity — per-piece spin guides](/advanced-velocity/static/angular-velocity.webp)
...
![Placing the blast source in the viewport](/advanced-velocity/static/point-source.png)
...
**Injecting Now** and **Muting Gravity** are live 0/1 readouts for driving an RBD solve, and **Create Connected RBD Sim** builds a solver already wired to both — see [Driving an RBD solver](/advanced-velocity/timed-events/#driving-an-rbd-solver).
...
![The Ballistic Motion folder on the Output tab](/advanced-velocity/static/ballistic-motion-parms.png)
...
![The Ballistic Return node](/advanced-velocity/static/ballistic-return-node.png)
