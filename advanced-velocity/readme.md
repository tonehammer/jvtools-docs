...
* **Solver-aware output** — Quick Setups configure the output for an RBD Bullet Solver (velocity) or POP / Vellum (accumulated force) in one click, with an **Injecting Now** signal for gating an RBD solve so physics runs free between impulses.
