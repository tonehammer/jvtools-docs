...
- 🔧 **TROUBLESHOOTING PAGES: ONE LAYOUT, AND ONLY REAL FAILURE MODES (Jovan, 2026-09-24).** Every page is `icon: question`, then entries as **bold symptom** — answer (the FLIP DOP Controller page is the reference), then `---` and the Discord line — **only if the product ships a Discord button** (all do since Hou-DisplacementX gained one in its 2026-09-25 rebuild). Cut anything that restates a tooltip, repeats a Using page, says "this is expected", or is an install step. **1-3 entries is right for a simple tool, and no page at all is fine** (Deoverlap Polylines and COPout have none). 🔑 **Reallusion Importer is EXEMPT** from the layout: it is the flagship and keeps its `##` Cause/Fix page (pruned with Jovan's sign-off the same day). The new-product template (`jvtools-internal/templates/new-product/docs-reference-troubleshooting.md.template`) carries the rule.
...
- 🔴 **A PRODUCT'S ARTWORK IS REFERENCED IN THREE PLACES — changing `index.yml` alone changes nothing you can see.** Swapping Advanced Velocity's icon in `advanced-velocity/index.yml` left the site looking untouched, because the two references that actually render were somewhere else:
  ...
  2. **`<product>/README.md` frontmatter `icon:`** — the section's landing page, which is what the sidebar entry shows. **This is the one that gets missed.**
     🔴 **It must be a FULL URL (`https://jvtools.dev/docs/<slug>/static/<file>`), not `static/<file>`** (2026-09-22). Retype's Pro "See also" backlink card on every sub-page copies this string verbatim, so a relative path resolves one folder too deep and 404s — 35 broken images across all seven sections. The sidebar takes the full URL as-is and loads it (verified). A local build cannot show the backlink card (Pro-only), so check a live sub-page after changing it.
...
- **Gumroad / purchase link (rule, Jovan 2026-07-22):** each product's Gumroad link goes as the **first element directly below the cover image** on its first page (README), not in the card. Format: `<p style="text-align:center; margin:0 0 1.5rem;"><a href="<gumroad-url>"><strong>Get it on Gumroad →</strong></a><br><a href="https://jvtools.dev/" style="font-size:0.9em;">More Houdini tools at jvtools.dev</a></p>`. URL source of truth = `versions/<product>.json` `url`. Skip the Gumroad half if that URL is a `PLACEHOLDER-*` (product not yet for sale).
  🔧 **The jvtools.dev line is the SECOND half of that same block and every landing page carries it** (Jovan, 2026-09-17). Reason: the product docs pages **outrank the website itself** in search, so these are the pages with traffic to hand on — the link exists to funnel it to the storefront, not for navigation (the header already has a Home link). Keep it secondary weight; the Gumroad CTA is the conversion and must stay the bold one. ⚠ It goes on the seven product landing pages, not on every page — a site-wide link would be navigation, which is the header's job.
...
- **`footer`** — site-wide footer links, **free, not Pro** (checked against
  retype.com, not assumed). Currently one Privacy link, added 2026-09-08.
  🔴 **Its URL is ABSOLUTE and must stay that way.** This build is served from
  two origins and `/privacy` exists on only one of them, so a root-relative
  link would 404 for every customer whose shipped HDA still opens the github.io
  copy. Everything else here is document-relative on purpose (that is what makes
  one artifact serve both prefixes); a link that resolves OFF the docs site is
  the one case that has to name its host. Full reasoning sits in `retype.yml`.
...
- 🔧 **EVERY changelog entry CARRIES ITS RELEASE DATE, as a muted line UNDER the heading — never inside it (2026-09-08).** The construction is two lines, heading then paragraph, with a blank line after it:
  ```
  ## Version 1.3
  <p style="font-size:0.75rem; font-weight:400; opacity:0.38; letter-spacing:0.02em; margin:-0.85rem 0 1.2rem;">30 August 2026</p>
  ```
  Copy the `<p>` verbatim from a neighbouring entry rather than retyping it, and use the long month (`8 September 2026`). **A release adds the date in the same edit as the entry**; all six products were backfilled on 2026-09-08.
  🔴 **The date must stay OUT of the `##` line, because the right-hand Contents rail is built from the heading text** — an inline span put a date on every TOC row and made the rail read as noise (Jovan, 2026-09-08: it shipped that way for an hour and he struck out every one of them). A separate block element is the only way to have the date on the page and not in the TOC.
  ⚠ **The blank line after `</p>` is load-bearing.** Without it the next markdown line is swallowed into the raw-HTML block and renders with its asterisks showing — it built clean and looked wrong only on screen, so screenshot after any edit to this pattern.
  🔑 **The dates are RECORDED, not remembered: `jvtools-website/src/data/changelog.mjs` carries a dated entry per release** and is where a backfill or a forgotten date comes from. Taking them from there also keeps the two surfaces agreeing by construction.
  ⚠ **An entry that was never a separate release gets NO date** — Advanced Velocity 1.0 is the case: 1.1 was the first public release and its own text says 1.0 is the feature set it launched with, so dating it would invent a release. ⚠ Reallusion 1.0's *20 June 2026* is approximate (it predates this site, so there is no commit to date it from) and is the figure the website already publishes; correct both together or neither.
