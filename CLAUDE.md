...
- 🔧 **TROUBLESHOOTING PAGES: ONE LAYOUT, AND ONLY REAL FAILURE MODES (Jovan, 2026-09-24).** Every page is `icon: question`, then entries as **bold symptom** — answer (the FLIP DOP Controller page is the reference), then `---` and the Discord line — **only if the product ships a Discord button** (Hou-DisplacementX deliberately does not). Cut anything that restates a tooltip, repeats a Using page, says "this is expected", or is an install step. **1-3 entries is right for a simple tool, and no page at all is fine** (Deoverlap Polylines and COPout have none). 🔑 **Reallusion Importer is EXEMPT** from the layout: it is the flagship and keeps its `##` Cause/Fix page (pruned with Jovan's sign-off the same day). The new-product template (`jvtools-internal/templates/new-product/docs-reference-troubleshooting.md.template`) carries the rule.
