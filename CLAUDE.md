...
- 🔧 **EVERY changelog heading CARRIES ITS RELEASE DATE, in a muted span (2026-09-08).** The heading is `## Version 1.3 <span style="font-size:0.58em; font-weight:400; opacity:0.5; letter-spacing:0.01em; margin-left:0.55em; white-space:nowrap;">— 30 August 2026</span>` — copy the span verbatim from a neighbouring entry rather than retyping it, and use the long month (`8 September 2026`). **A release adds the date in the same edit as the entry**; all six products were backfilled on 2026-09-08. The date lands in the right-hand Contents rail too, so the TOC reads as a dated release list.
