...
**Reallusion Importer for Houdini** (v1.2.2) turns a Character Creator 5 or iClone 8 character into a fully shaded, animatable, render-ready character in Houdini's Solaris environment — in seconds instead of hours.
...
Built for Character Creator 5 / iClone 8, working with all CC3+ characters. Ships as a black-boxed Houdini Digital Asset, tuned for Karma XPU and also renders in Karma CPU.
...
* **Karma XPU materials out of the box.** Physically-based MaterialX skin (with subsurface scattering), refractive eyes, teeth, and transparent card hair.
...
* **Flexible animation.** Add multiple motion clips and switch between them, retarget clips from differently-proportioned characters, and drive the body from one clip while keeping another's facial performance.
* **Skin Fix.** Clean up mesh intersections — clothing poking through the body — with built-in Sculpt and Soft Edit tools, right inside the asset.
* **Optional render setup.** A three-point light rig, camera, and Karma render settings, ready to go.

## Who it's for

Indie character artists, animators, and small studios who use Character Creator or iClone and want to render their characters beautifully in Houdini and Karma — without becoming a lookdev TD first.

You don't need to know how MaterialX works, or understand subsurface scattering or anisotropic hair shading. The tool handles the hard parts and gives you simple sliders and color pickers for the creative parts.
...
After that, **Using the Asset** explains every control in detail, and **Reference** covers performance, troubleshooting, and what's new in each version.
