---
label: Products
icon: home
order: 10000
---

# JVtools

Houdini digital assets and tools for artists and technical directors. Pick a product below for its documentation.

## Products

[!card title="Reallusion Importer for Houdini" image="/reallusion-importer/static/card.png" text="Turn a Character Creator 5 / iClone 8 FBX character into a fully shaded, animatable, render-ready Solaris character with Karma XPU MaterialX materials — in one click."](/reallusion-importer/README.md)

[!card title="RPLidar In" icon=":satellite:" text="Bring live Slamtec RPLIDAR point-cloud data into Houdini as SOP geometry — stream, record, visualize, and drive live simulations."](/rplidar-in/README.md)

[!card title="Advanced Velocity" image="/advanced-velocity/static/card.png" text="Author @v for simulations on one node — fixed, directional, exploding and angular velocity, with matching Adjust and Mask controls and interactive blast placement for fractured RBD."](/advanced-velocity/README.md)

---

More tools coming. Each product has its own section with a full guide, parameter reference, and troubleshooting.
