...
- **Version in title (rule, Jovan 2026-07-22):** each product's first-page **H1 ends with the current version**, e.g. `# Advanced Velocity v1.0`. **Source of truth = the product's changelog page** — the top-most `## Version X.Y.Z` heading (e.g. `reference/changelog.md` for Reallusion, `changelog.md` elsewhere). The `versions/<product>.json` `latest` field is a *separate* thing (it feeds the HDAs' in-app update check, not the docs) and can lag — if it disagrees with the changelog, trust the changelog and flag the manifest.
